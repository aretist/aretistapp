...
export default function EducationSection({ education, userId }: Props) {
  ...
  async function handleUpdate() {
    if (!institution || !title || !startDate || !editingId) return
    setSaving(true)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: 16, background: 'var(--bg-surface)', borderRadius: 'var(--radius-md)', border: '0.5px solid var(--border)' }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button
          ...
          style={{ flex: 2, padding: '10px', background: 'var(--red)', color: 'white', border: 'none', borderRadius: 'var(--radius-full)', fontFamily: 'var(--font)', fontSize: 14, fontWeight: 500, cursor: 'pointer', opacity: saving || !institution || !title || !startDate ? 0.5 : 1 }}
        >
          {saving ? 'Guardando...' : editingId ? 'Guardar cambios' : 'Guardar formación'}
        </button>
      </div>
    </div>
  )
          ...
          <div key={edu.id}>
            {editingId === edu.id ? (
              ...
              <div style={{ padding: '12px 16px', background: 'white', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-md)', position: 'relative' }}>
                ...
                <p style={{ fontWeight: 600, fontSize: 14, fontFamily: 'var(--font)', color: 'var(--text-primary)', paddingRight: 100 }}>{edu.title}</p>
