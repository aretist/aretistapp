...
export default async function JobsPage({
  ...
  if (params.duration) query = query.eq('duration_type', params.duration)
  ...
  const [
    ...
    employerIds.length
      ? supabase.from('users').select('id, full_name, username, avatar_url').in('id', employerIds)
      : Promise.resolve({ data: [] }),
    employerIds.length
      ? supabase.from('employer_profiles').select('user_id, company_name, logo_url, verified').in('user_id', employerIds)
      : Promise.resolve({ data: [] }),
    supabase.from('job_applications').select('job_id, status, saved').eq('applicant_id', user.id),
  ...
  return (
    <div style={{ maxWidth: 640, margin: '40px auto', padding: '0 20px 80px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        {isEmployer && (
          <Link href="/jobs/new" style={{ padding: '8px 16px', background: 'var(--red)', color: 'white', borderRadius: 6, fontSize: 13, fontWeight: 500, textDecoration: 'none' }}>
            + Publicar oferta
          </Link>
        )}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 20 }}>
        ...
        {jobsWithEmployer.map((job: any) => (
          <JobCard key={job.id} job={job} application={applicationMap.get(job.id) ?? null} />
        ))}
      </div>
    </div>
  )
